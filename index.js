...
for (let i = 0; i < rows.length; i++) {
  ...
  companyElement.textContent = `Company Name: ${companyName}`;

  const jobProfileElement2 = document.createElement("p");
  jobProfileElement2.textContent = `Job Profile: ${jobProfile}`;

  const salaryCtcElement = document.createElement("p");
  salaryCtcElement.textContent = `Salary/CTC: ${salaryCtc}`;

  // Append the created elements to the results container
  resultsContainer.appendChild(companyElement);
  resultsContainer.appendChild(jobProfileElement);
  resultsContainer.appendChild(salaryCtcElement);
